    ...
    if(notification){
        ...
        if(notification.name == UIKeyboardWillShowNotification || notification.name == UIKeyboardDidShowNotification){
            if(ignoreOrientation || UIInterfaceOrientationIsPortrait(orientation)){
                svkeyboardHeight = CGRectGetHeight(keyboardFrame);
            } else{
                svkeyboardHeight = CGRectGetWidth(keyboardFrame);
            }
        }
    } else{
        svkeyboardHeight = self.visibleKeyboardHeight;
    }
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle{
    ...
    self.hudView.center = CGPointMake(newCenter.x + self.offsetFromCenter.horizontal, newCenter.y + self.offsetFromCenter.vertical + 100);
}
